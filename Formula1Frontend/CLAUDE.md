# Mendix Project: Formula1Frontend
...
```bash
./mxcli -p Formula1Frontend.mpr    # Correct - uses local binary
```
...
```bash
./mxcli setup mxbuild -p Formula1Frontend.mpr    # Auto-download for project's Mendix version
```

After setup, `mx` is at `~/.mxcli/mxbuild/{version}/modeler/mx`. Usage:

```bash
~/.mxcli/mxbuild/*/modeler/mx check Formula1Frontend.mpr   # Validate project
./mxcli docker check -p Formula1Frontend.mpr               # Alternative (auto-downloads mxbuild)
```
...
```bash
./mxcli -p Formula1Frontend.mpr -c "SHOW MODULES"              # List all modules
./mxcli -p Formula1Frontend.mpr -c "SHOW STRUCTURE"             # Project overview
./mxcli -p Formula1Frontend.mpr -c "SHOW ENTITIES IN MyModule"  # Entities in a module
./mxcli -p Formula1Frontend.mpr -c "DESCRIBE ENTITY MyModule.Customer"  # Entity details
```

### Execute an MDL Script File

```bash
./mxcli exec script.mdl -p Formula1Frontend.mpr
```
...
```bash
./mxcli
# Then: CONNECT LOCAL 'Formula1Frontend.mpr';
```
...
./mxcli check script.mdl -p Formula1Frontend.mpr --references  # With reference validation
```
...
```bash
./mxcli check script.mdl -p Formula1Frontend.mpr --references
```
...
```bash
# Lint the project
./mxcli lint -p Formula1Frontend.mpr

# With colored output
./mxcli lint -p Formula1Frontend.mpr --color

# List available rules
./mxcli lint -p Formula1Frontend.mpr --list-rules

# Output as SARIF
./mxcli lint -p Formula1Frontend.mpr --format sarif > results.sarif
```
...
./mxcli report -p Formula1Frontend.mpr

# JSON report
./mxcli report -p Formula1Frontend.mpr --format json

# HTML report
./mxcli report -p Formula1Frontend.mpr --format html
```
...
| Enum split (CASE) | `CASE $Var/Attr AS x WHEN Enum.Value THEN ... END CASE` |
