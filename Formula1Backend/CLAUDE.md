# Mendix Project: Formula1Backend
...
```bash
./mxcli -p Formula1Backend.mpr    # Correct - uses local binary
```
...
```bash
./mxcli setup mxbuild -p Formula1Backend.mpr    # Auto-download for project's Mendix version
```

After setup, `mx` is at `~/.mxcli/mxbuild/{version}/modeler/mx`. Usage:

```bash
~/.mxcli/mxbuild/*/modeler/mx check Formula1Backend.mpr   # Validate project
./mxcli docker check -p Formula1Backend.mpr               # Alternative (auto-downloads mxbuild)
```
...
```bash
./mxcli -p Formula1Backend.mpr -c "SHOW MODULES"              # List all modules
./mxcli -p Formula1Backend.mpr -c "SHOW STRUCTURE"             # Project overview
./mxcli -p Formula1Backend.mpr -c "SHOW ENTITIES IN MyModule"  # Entities in a module
./mxcli -p Formula1Backend.mpr -c "DESCRIBE ENTITY MyModule.Customer"  # Entity details
```

### Execute an MDL Script File

```bash
./mxcli exec script.mdl -p Formula1Backend.mpr
```
...
```bash
./mxcli
# Then: CONNECT LOCAL 'Formula1Backend.mpr';
```
...
./mxcli check script.mdl -p Formula1Backend.mpr --references  # With reference validation
```
...
```bash
./mxcli check script.mdl -p Formula1Backend.mpr --references
```
...
```bash
# Lint the project
./mxcli lint -p Formula1Backend.mpr

# With colored output
./mxcli lint -p Formula1Backend.mpr --color

# List available rules
./mxcli lint -p Formula1Backend.mpr --list-rules

# Output as SARIF
./mxcli lint -p Formula1Backend.mpr --format sarif > results.sarif
```
...
./mxcli report -p Formula1Backend.mpr

# JSON report
./mxcli report -p Formula1Backend.mpr --format json

# HTML report
./mxcli report -p Formula1Backend.mpr --format html
```
