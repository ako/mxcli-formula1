...
"""Minimal OTLP/HTTP trace collector.
    ...
    python3 scripts/otlp-collector.py [out.jsonl] [port]     # default /tmp/spans.jsonl 4318

Why this exists: `mxcli run --trace` attaches the OpenTelemetry agent with a
*console* exporter, and that exporter drops start/end timestamps and parent span
ids — enough to see that a span happened, not enough for a duration or a call
tree. `--trace-otlp <endpoint>` fixes that but needs somewhere to send to. This
is that somewhere, in one file with no service to run: it accepts
POST /v1/traces (protobuf, optionally gzipped), decodes it, and appends one JSON
object per span to a file that `scripts/span-report.py` reads.

Both apps can export here at once — every span carries its service.name, so the
backend's SQL and the frontend's OData calls stay distinguishable in one file.
"""
import gzip
import json
import sys
